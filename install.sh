...
config_after_install() {
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_username" == "admin" && "$existing_password" == "admin" ]]; then
            ...
            read -p "是否自定义面板端口设置？（如果不设置，将会应用一个随机端口）[y/n]: " config_confirm
            ...
            echo -e "这是一次新安装，出于安全考虑生成了随机的登录信息："
            echo -e "###############################################"
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}下载x-ui失败，请确保您的服务器能够访问GitHub ${plain}"
            exit 1
        fi
    else
        tag_version=$1
